...
class Search_model extends \Mobiledrs\core\MY_Models {
	...
	private $access_modules = [
		...
		'1000000001' => ['patient', 'provider', 'facilities', 'user', 'routesheet'],
		'2000000002' => ['patient', 'provider', 'facilities', 'user', 'routesheet'],
		'1000000002' => ['patient', 'provider', 'facilities', 'user', 'routesheet'],
		'2000000003' => ['patient', 'provider', 'facilities', 'routesheet'],
		'1000000003' => ['patient', 'provider', 'facilities', 'routesheet']
	];
	...
	{
		$this->db->join(
			'routesheet.provider_route_sheet_list',
			'routesheet.provider_route_sheet_list.prsl_prsID = routesheet.provider_route_sheet.prs_id',
			'left'
		);

		$this->db->join(
			'gimg.provider',
			'provider.provider_id = routesheet.provider_route_sheet.prs_providerID',
			'left'
		);

		$this->db->join(
			'gimg.patient',
			'gimg.patient.patient_id = routesheet.provider_route_sheet_list.prsl_patientID',
			'left'
		);

		$this->db->like('routesheet.provider_route_sheet.prs_dateOfService', $searchTerm);

		$this->db->or_like('routesheet.provider_route_sheet_list.prsl_fromTime', $searchTerm);
		$this->db->or_like('routesheet.provider_route_sheet_list.prsl_toTime', $searchTerm);
		$this->db->or_like('routesheet.provider_route_sheet_list.prsl_notes', $searchTerm);
		$this->db->or_like('routesheet.provider_route_sheet_list.prsl_dateRef', $searchTerm);

		$this->db->or_like('gimg.provider.provider_firstname', $searchTerm);
		$this->db->or_like('gimg.provider.provider_lastname', $searchTerm);
		$this->db->or_like("CONCAT_WS(' ', gimg.provider.provider_firstname, gimg.provider.provider_lastname)", $searchTerm);

		$this->db->or_like('patient.patient_name', $searchTerm);

		$this->db->group_by("routesheet.provider_route_sheet_list.prsl_prsID");

		$query = $this->db->get('routesheet.provider_route_sheet');
