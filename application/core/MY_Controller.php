...
class MY_Controller extends \CI_Controller {
	...
	public function is_logged_in()
	{
        ...
        if (empty($this->session->userdata('user_db')))
        {
            return redirect('authentication/user/logout');
        }

        $user_entity = $this->user_model->record($user_params, $this->session->userdata('user_db'));
	...
	{
        $has_permission = $this->acl->has_permission(
            $this->session->userdata('user_roleID'), 
            $permission_name, 
            $this->session->userdata('user_db')
        );
